...
struct FGPUMeshParameters
{
	UTextureRenderTarget2D* InputSDFImage;
	...
	FGPUMeshParameters() {}
	FGPUMeshParameters(UTextureRenderTarget2D* _Output)
		:OutputVertexPositionImage(_Output)
	{
		CachedRenderTargetSize = _Output ? FIntPoint(_Output->SizeX, _Output->SizeY) : FIntPoint::ZeroValue;
	}
private:
	FIntPoint CachedRenderTargetSize;
};
...
private:
	...
	TRefCountPtr<IPooledRenderTarget> GPUMeshShaderOutput;
